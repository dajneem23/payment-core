...
// The SDK monkey-patches http/express at require-time so spans
...
async function bootstrap() {
    ...
    const port = configService.getNumber('PORT') || 3006;
    const host = configService.get('HOST') || '127.0.0.1';
    await app.listen(port, host);

    loggerService.warn(`acquirer-sim running on ${host}:${port}`);
}
bootstrap();
